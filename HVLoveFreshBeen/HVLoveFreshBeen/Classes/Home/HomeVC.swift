...
class HomeVC: SelectedAdressViewController {
    
    fileprivate var headView: HomeHeadView?
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        navigationController?.navigationBar.barTintColor = BaseNavigationYellowColor
    ...
    func addHomeNotification() {
        NotificationCenter.default.addObserver(self, selector: #selector(HomeHeadViewHeightDidChange(_:)), name: NSNotification.Name(rawValue: HomeTableHeadViewHeightDidChange), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(goodsInventoryProblem(_:)), name: NSNotification.Name(rawValue: HomeGoodsInventoryProblem), object: nil)
    ...
    fileprivate func buildCollectionView() {
        ...
        collectionView.register(HomeCollectionHeadView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "headerView")
        collectionView.register(HomeCollectionFootView.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: "footerView")
        view.addSubview(collectionView)
        
        let refreshHeadView = HVRefreshHeader(refreshingTarget: self, refreshingAction: #selector(headRefresh))
    ...
    @objc func HomeHeadViewHeightDidChange(_ noti: Notification) {
        collectionView!.contentInset = UIEdgeInsets(top: noti.object as! CGFloat, left: 0, bottom: NavigationBarHeight, right: 0)
...
extension HomeVC: HomeTableHeadViewDelegate {
    func tableHeadView(_ headView: HomeHeadView, focusImageViewClick index: Int) {
...
//        }
    ...
    func tableHeadView(_ headView: HomeHeadView, iconClick index: Int) {
...
extension HomeVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if (indexPath as NSIndexPath).section == 1 && kind == UICollectionView.elementKindSectionHeader {
            let headView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "headerView", for: indexPath) as! HomeCollectionHeadView
            
            return headView
        }
        
        let footerView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: "footerView", for: indexPath) as! HomeCollectionFootView
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(moreGoodsClick(_:)))
...
//            navigationController?.pushViewController(productVC, animated: true)
    ...
    @objc func moreGoodsClick(_ tap: UITapGestureRecognizer) {
        if tap.view?.tag == 100 {
            let tabBarController = UIApplication.shared.keyWindow?.rootViewController as! BaseTabBarController
